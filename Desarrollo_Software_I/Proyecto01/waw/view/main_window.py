...
class MainWindow(tk.Frame):
    def __init__(self, master, controller):
        ...
        # self.input.bind("<Key>", self.handle_keypress)
        # self.master.bind("<Control-u>", self.undo_operation())
        # self.master.bind("<Control-r>", self.redo_operation())
    ...
    def handle_keypress(self, event):
        ...
        if key.isdigit() or key in "+-*/().{}[]":
            ...
            self.calculate()
        # elif '<Control-z>':  # Ctrl+Z para deshacer
        #     self.undo_operation()
        # elif event.state == 4 and key.lower() == 'y':  # Ctrl+Y para rehacer
        #     self.redo_operation()
