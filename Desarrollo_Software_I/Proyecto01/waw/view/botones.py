...
class Botones():
    def __init__(self, master, undo_redo, controller,  input, output):
        ...
        self.botonUndo = tk.Button(botonesAccionFrame,state="normal", text="⟲", font=('Cartograph CF', 21, 'normal'), bg=bg_accion, fg=foreground, padx=27, pady=16, borderwidth=0, command= self.undo_operation)
        self.botonUndo.grid(row=0, column=0, padx=2, pady=2)

        self.botonRedo = tk.Button(botonesAccionFrame, state="normal", text="⟳", font=('Cartograph CF', 21, 'normal'), bg=bg_accion, fg=foreground, padx=27, pady=16, borderwidth=0, command=self.redo_operation)
        self.botonRedo.grid(row=0, column=1, padx=2, pady=2)

        self.botones_controller = BotonesController(master, undo_redo, controller,  input, output, self.botonUndo, self.botonRedo)
        ...
        self.botones_controller.master.bind("<Key>", self.botones_controller.handle_keypress)

        self.botones_controller.master.bind("<Control-z>", self.botones_controller.undo_operation)
        self.botones_controller.master.bind("<Control-y>", self.botones_controller.redo_operation)

    def undo_operation(self):
        self.botones_controller.undo_operation(self)

    def redo_operation(self):
        self.botones_controller.redo_operation(self)
